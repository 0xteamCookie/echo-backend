...
function parseRole(
  value: unknown,
): "super_admin" | "medical" | "fire" | "police" | null {
...
export const provisionController = {
  issueToken: (async (req, res) => {
    ...
    if (typeof sub !== "string" || !sub.trim()) {
      res
        .status(400)
        .json({ error: "sub is required and must be a non-empty string" });
      return;
    }
    if (!role) {
      res.status(400).json({
        error: "role must be one of super_admin, medical, fire, police",
      });
      return;
    }
    if (!agency) {
      res
        .status(400)
        .json({ error: "agency must be one of medical, fire, police" });
      return;
    }
    if (role !== "super_admin" && role !== agency) {
      res.status(400).json({
        error: "agency-scoped users must have matching role and agency",
      });
      return;
    }
    if (typeof body.name !== "string" || !body.name.trim()) {
      res.status(400).json({
        error: "name is required and must be a non-empty string (agent name)",
      });
      return;
    }
    ...
    if (radius_m === null || radius_m <= 0) {
      res
        .status(400)
        .json({ error: "radius_m must be a positive number (metres)" });
      return;
    }
    ...
    if (lat === null || lat < -90 || lat > 90) {
      res
        .status(400)
        .json({ error: "lat must be a number between -90 and 90" });
      return;
    }
    ...
    if (lng === null || lng < -180 || lng > 180) {
      res
        .status(400)
        .json({ error: "lng (or long) must be a number between -180 and 180" });
      return;
    }
    ...
    if (body.expiresInSeconds !== undefined) {
      if (
        ...
        res
          .status(400)
          .json({ error: "expiresInSeconds must be a finite number" });
        return;
      }
    ...
    try {
      ...
      if (message.includes("FIREBASE_SERVICE_ACCOUNT_JSON")) {
        res.status(503).json({
          error:
            "JWT provisioning is not configured (FIREBASE_SERVICE_ACCOUNT_JSON missing or invalid)",
        });
        return;
      }
      throw e;
